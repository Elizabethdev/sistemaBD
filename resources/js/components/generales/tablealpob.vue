        ...
        <tbody>
            <template v-for="(value, index) in datosTotales">
                <tr v-if="value.TIPO_20 == 'URBANA' || value.TIPO_20 == 'RURAL'" :key="value.cve_u">
                    ...
                    <td>{{value.TIPO_20}}</td>
                    <td>{{formatNumber(value.PO_CON_ALC_10)}}</td>
                    <td>{{formatNumber(value.PO_SIN_ALC_10)}}</td>
                    <td>{{formatNumber(value.PO_CON_ALC_15)}}</td>
                    <td>{{formatNumber(value.PO_SIN_ALC_15)}}</td>
                    <td>{{formatNumber(value.PO_CON_ALC_20)}}</td>
                    <td>{{formatNumber(value.PO_SIN_ALC_20)}}</td>
                    <td>{{formatNumber(value.PO_CON_ALC_30)}}</td>
                    <td>{{formatNumber(value.PO_SIN_ALC_30)}}</td>
